...
argument-hint: "<component-name> [variants] [sizes] [states] (for example: ds-button primary,secondary sm,md,lg disabled,loading)"
---

# Create Angular Design-System Component

Create enterprise-ready components for `@my-org/design-system`. Follow this workflow exactly. The source library is `projects/design-system`; never create a parallel `libs/ui` tree.
...
8. Validate in this order and fix failures before continuing:

```bash
npm run build:tokens
npx ng test design-system --watch=false
npm run lint
npm run build:lib
...
On Windows PowerShell, invoke `npm.cmd` and `npx.cmd` instead. Do not use the raw Storybook CLI; use the Angular architect-backed npm scripts configured by the workspace.
